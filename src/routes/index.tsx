...
import { ArrowRight, Check, Leaf, Menu, Minus, Plus, ShoppingBag, ShieldCheck, Truck } from "lucide-react";
...
function Index() {
  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <section className="border-b border-border bg-card"><div className="mx-auto grid max-w-7xl grid-cols-1 divide-y divide-border px-5 sm:grid-cols-3 sm:divide-x sm:divide-y-0 sm:px-8">{[[Truck,"Envio gratuito","Portugal Continental"],[ShieldCheck,"Compra tranquila","14 dias para devolver"],[Leaf,"Feito para durar","Capa lavável e resistente"]].map(([Icon,title,text]) => { const I = Icon as typeof Truck; return <div key={title as string} className="flex items-center gap-4 py-6 sm:px-8 sm:first:pl-0"><I className="size-5 text-accent"/><div><p className="text-sm font-semibold">{title as string}</p><p className="text-xs text-muted-foreground">{text as string}</p></div></div> })}</div></section>
    ...
    <section id="perguntas" className="mx-auto grid max-w-5xl gap-12 px-5 py-20 sm:px-8 sm:py-28 md:grid-cols-[0.8fr_1.2fr]"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-accent">Antes de ir</p><h2 className="mt-3 font-display text-5xl">Perguntas frequentes.</h2></div><Accordion type="single" collapsible>{[["O puffe pode molhar-se?","A capa exterior repele salpicos e seca rapidamente. Para preservar o enchimento, não deve ficar submerso nem passar a noite à chuva."],["Como se lava?","Retire a capa pelo fecho protegido e lave a 30 ºC, num programa delicado. Deixe secar naturalmente."],["O enchimento está incluído?","Sim. Todos os puffes Moka são entregues completos e prontos a usar."],["Quanto demora a entrega?","Em Portugal Continental, a entrega estimada é de 2 a 4 dias úteis após expedição."]].map(([q,a]) => <AccordionItem value={q} key={q}><AccordionTrigger className="py-6 text-base hover:no-underline">{q}</AccordionTrigger><AccordionContent className="pb-6 leading-6 text-muted-foreground">{a}</AccordionContent></AccordionItem>)}</Accordion></section>
